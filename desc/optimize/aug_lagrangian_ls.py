        ...
        if Lactual_reduction > 0:
            ...
            if g_norm < gtolk:
                ...
                if constr_violation < ctolk:
                    ...
                    ctolk = max(eta / (jnp.mean(mu) ** alpha_eta), ctol)
                    gtolk = max(omega / (jnp.mean(mu) ** alpha_omega), gtol)
                # if we update lagrangian params, need to recompute L, and rescale J.
                # lagjac stacks [Jf; sqrt(mu) * Jc] and neither block depends on y or
                # mu, so scaling the constraint rows by sqrt(mu_new)/sqrt(mu_old) is
                # exactly a recomputation at the current z, without the Jacobian
                # evaluation (so njev does not increment here).
                L = lagfun(f, c, y, mu)
                Lcost = 0.5 * jnp.dot(L, L)
                d_rows = jnp.concatenate(
                    [jnp.ones(f.size), jnp.sqrt(mu) / jnp.sqrt(mu_old)]
                )
                J = scale_matrix(J, d_rows[:, None])
